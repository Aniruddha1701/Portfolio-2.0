...
const GlassDock: React.FC<{ icons: DockIcon[]; href?: string }> = ({
  ...
  <GlassEffect
    href={href}
    className="rounded-3xl p-2 hover:p-3 hover:rounded-[2rem]"
  >
    <div className="flex flex-col items-center justify-center gap-2 rounded-3xl p-2 overflow-hidden">
      {icons.map((icon, index) => (
        <img
          key={index}
          src={icon.src}
          alt={icon.alt}
          className="w-10 h-10 transition-all duration-700 hover:scale-110 cursor-pointer"
          style={{
